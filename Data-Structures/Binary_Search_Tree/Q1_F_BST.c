...
int main() {
    ...
    while (c != 0) {
        ...
        switch (c) {
        case 1:
            printf("Input an integer that you want to insert into the Binary "
                   "Search Tree: ");
            scanf("%d", &i);
            insertBSTNode(&root, i);
            break;
        case 2:
            printf("The resulting level-order traversal of the binary search "
                   "tree is: ");
...
void levelOrderTraversal(BSTNode *root) {
    /* add your code here */
    // 루트 노드 레벨부터 시작하여 큐를 사용해 이진트리를 레벨별로 순회

    // 의사코드
    // 큐 선언(head, tail NULL 빈 큐)
    // bst의 root 노드를 큐에 enqueue()
    // while(큐가 비어있지 않을때동안)
    // 큐에서 dequeue(), 출력
    // 해당 노드의 left, right 체크
    // 각각 존재하면 left, right를 큐에 enqueue()
    // 여기까지 반복
    ...
    while (!isEmpty(q.head)) {
        BSTNode *popNode = dequeue(&q.head, &q.tail);
