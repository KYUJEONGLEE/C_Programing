...
void createQueueFromLinkedList(LinkedList *ll, Queue *q)
{
	...
	while (temp != NULL)
	{
		// 연결리스트의 첫번째 노드부터 Queue 에 enque 한다.
		// while temp가 null X
		// enqueue(temp)
		// temp = temp.next
		enqueue(q, temp->item);
...
void removeOddValues(Queue *q)
{
	...
	// queue 전체를 순회하는 방법
	// size가 deque 할때마다 동적으로 변해버려서 반복문을 끝까지 못돈다.
	// 변하게 하지 못하는 방법이 있을까?
	// 반복문 바깥에서 size 변수를 선언한다.
	int size = (q->ll).size;

	// 핵심 로직
	// 1) deque(q)
	// 2) dequeue 한 원소 짝수,홀수 여부 판단
	// 3) 짝수면 다시 queue 에 enque()
	// 4) 처음 큐의 크기만큼 반복
